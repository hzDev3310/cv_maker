...
export function setActiveProvider(providerId) {
  try {
    localStorage.setItem(ACTIVE_PROVIDER_KEY, getProvider(providerId).id);
    return true;
  } catch {
    return false;
  }
...
export function getProviderModel(providerId) {
  ...
  try {
    const stored = localStorage.getItem(`${ACTIVE_MODEL_PREFIX}${provider.id}`);
...
export function setProviderModel(providerId, modelId) {
  const provider = getProvider(providerId);
  const valid = provider.models.some((model) => model.id === modelId);
  const next = valid ? modelId : provider.defaultModel;
  try {
    localStorage.setItem(`${ACTIVE_MODEL_PREFIX}${provider.id}`, next);
    return true;
  } catch {
    return false;
  }
...
export function saveApiKey(key, providerId = getActiveProvider()) {
  const provider = getProvider(providerId);
  try {
    if (!key) {
...
async function callProvider(systemPrompt, userPrompt, { temperature = 0.4, providerId = getActiveProvider(), modelId } = {}) {
  const provider = getProvider(providerId);
  ...
  try {
    const res = await fetch(provider.endpoint, {
      ...
      headers: {
        ...
        [provider.authHeader]: `Bearer ${key}`,
      },
      body: JSON.stringify({
        model: modelId || getProviderModel(provider.id),
        messages: [
 ...
 * @param {string} [opts.providerId] - active provider id
 * @param {string} [opts.modelId] - optional override model id
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const reminder = attempt > 0
        ...
        : "";
      const raw = await callProvider(systemPrompt + reminder, userPrompt, { temperature, providerId, modelId });
      lastRaw = raw;
...
export function classifyError(code, providerLabel = "AI") {
  switch (code) {
    case "MISSING_KEY": return { icon: "🔑", message: `Add your ${providerLabel} API key in Settings to use this feature.`, action: "settings" };
    case "INVALID_KEY": return { icon: "❌", message: `${providerLabel} API key is invalid. Check Settings.`, action: "settings" };
